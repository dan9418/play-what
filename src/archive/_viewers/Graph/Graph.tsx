...
const Label = ({ axis, children }) => {
  return <div className={`label ${axis}`}>{children}</div>;
};
...
const getCells = (origin, pods) => {
  ...
  for (let p = MAX_POD[0] - 1; p >= -1; p--) {
    ...
    for (let d = 0; d < MAX_POD[1]; d++) {
      if (p === -1) {
        cells.push(
          <Label key={"d" + d} axis="x">
            {d}
          </Label>
        );
      } else {
        const point: any = [p, d];
        const isOrigin = ModelUtils.areEqual(origin, point);
        const isResultant =
          pods.findIndex((v) => ModelUtils.areEqual(v, point)) >= 0;
        cells.push(
          <Cell
            key={d + "-" + p}
...
const Graph = (userProps) => {
  ...
  const { origin, title, xLabel, yLabel, pods, presetId } = props;
  return (
